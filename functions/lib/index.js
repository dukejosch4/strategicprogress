...
exports.triggerWhoopSync = exports.scheduledWhoopFetch = exports.whoopCallback = exports.whoopAuth = void 0;
 ...
 */
exports.whoopAuth = functions.https.onRequest((req, res) => {
...
exports.whoopCallback = functions.https.onRequest(async (req, res) => {
    ...
    try {
        ...
        const tokenResponse = await axios_1.default.post("https://api.prod.whoop.com/oauth/oauth2/token", qs.stringify({
...
async function getAccessToken() {
    ...
    if (!doc.exists)
        throw new Error("No tokens found. Authenticate first via whoopAuth.");
    const data = doc.data();
    const now = Date.now();
    ...
    if (data.expiresAt.toMillis() < now + 5 * 60 * 1000) {
        ...
        const CLIENT_SECRET = functions.config().whoop.client_secret;
        try {
            const response = await axios_1.default.post("https://api.prod.whoop.com/oauth/oauth2/token", qs.stringify({
 ...
 */
exports.scheduledWhoopFetch = functions.pubsub.schedule('every 24 hours').onRun(async (context) => {
    await syncWhoopData();
});
/**
 ...
 */
exports.triggerWhoopSync = functions.https.onRequest(async (req, res) => {
    // Enable CORS
...
async function syncWhoopData() {
    try {
        ...
        const getDateStr = (isoString) => isoString ? isoString.split('T')[0] : null;
        ...
        for (const rec of recoveries) {
            ...
            if (!date)
                continue;
            if (!metricsMap[date])
                metricsMap[date] = { date: admin.firestore.Timestamp.fromDate(new Date(date)) };
            if (rec.score) {
